// pages/microConsultation/ques/ques.js
const app=getApp();
Page({
   ...
   */
  onUnload: function () {
    // if (this.data.content == '' || this.data.imgList == '' || this.data.industrySelId == '') {
    //   app.ajax_nodata("/applet/addQuestionNum", function (res) { })
    // }
        ...
        app.ajax_wzx("/question/releaseQuestion/third", {
          ...
          if (res.data.code == 1) {
            ...
            app.ajax_nodata("/applet/reduceQuestionNum", function (res) {
              app.ajax_nodata("/applet/addQuestionRecord", function (res) {
                that.setData({
                  ifClick:true
                })
                wx.navigateBack();
  ...
  imgLook:function(e){
    var that=this;
    var imgLook = [], i, imgList = that.data.imgList ;
    for (i in imgList){
